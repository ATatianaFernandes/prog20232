#três regiões aleatórias do Brasil
...
# Definindo a tupla com os dados
dados_covid = (
    ("18/08/2020", "RJ", "Rio de Janeiro", "Rio de Janeiro", 123456, 7890),
    ("18/08/2020", "SP", "São Paulo", "São Paulo", 456789, 12345),
    ("18/08/2020", "PB", "Paraíba", "João Pessoa", 9876, 5432),
    ("18/08/2020", "PB", "Paraíba", "Campina Grande", 5678, 1234),
)

# Definindoo a lista a partir da tupla
dado_sda_covid_lista = list(dados_da_covid)

# b. Printar casos acumulados para o estado do Rio de Janeiro
casos_rj_tupla = None
casos_rj_lista = None

for data, estado, _, _, casos_acumulados, _ in dados_covid:
    if estado == "RJ":
        casos_rj_tupla = casos_acumulados

for _, estado, _, _, casos_acumulados, _ in dados_covid_lista:
    if estado == "RJ":
        casos_rj_lista = casos_acumulados

print("Casos acumulados para o estado do Rio de Janeiro (tupla):", casos_rj_tupla)
print("Casos acumulados para o estado do Rio de Janeiro (lista):", casos_rj_lista)

# c. Printar óbitos acumulados para estados
obitos_por_estado = {}
for _, estado, _, _, _, obitos_acumulados in dados_covid:
    if estado not in obitos_por_estado:
        obitos_por_estado[estado] = obitos_acumulados
    else:
        obitos_por_estado[estado] += obitos_acumulados

print("Óbitos acumulados por estado:")
for estado, obitos in obitos_por_estado.items():
    print(f"{estado}: {obitos}")

# d. Corrigir os dados de óbitos novos para a Paraíba
obitos_paraiba_corrigidos = 5432 + 10  # Correção dos dados

for i, (data, estado, regiao, municipio, casos_acumulados, obitos_acumulados) in enumerate(dados_covid):
    if estado == "PB":
        dados_covid[i] = (data, estado, regiao, municipio, casos_acumulados, obitos_paraiba_corrigidos)

for i, (data, estado, regiao, municipio, casos_acumulados, obitos_acumulados) in enumerate(dados_covid_lista):
    if estado == "PB":
        dados_covid_lista[i] = (data, estado, regiao, municipio, casos_acumulados, obitos_paraiba_corrigidos)

# e. As duas operações foram possíveis (lista e tupla)? Justifique.
Sim, ambas as operações foram possíveis. Você pode modificar os elementos individuais de uma lista ou tupla, desde que você tenha acesso a esses elementos, o que é o caso aqui.

# f. Criar uma nova lista com dados de 1 estado e adicionar à lista existente
novo_estado = [
    ("18/08/2020", "BA", "Bahia", "Salvador", 1111, 222)
]

dados_covid_lista.extend(novo_estado)

# g. Remover dados das regiões de saúde
dados_covid_lista = [dados for dados in dados_covid_lista if dados[2] != "Região de Saúde"]

# h. Verificar a soma dos dados dos municípios na data de 18/08/2020
soma_municipios = sum(casos_acumulados for data, _, _, _, casos_acumulados, _ in dados_covid_lista if data == "18/08/2020")

print("Soma dos casos acumulados nos municípios em 18/08/2020 é igual ao dado da lista:", soma_municipios == casos_rj_lista)

# i. Retornar o tamanho total da lista
tamanho_lista = len(dados_covid_lista)

print("Tamanho total da lista:", tamanho_lista)

# j. Encontrar o maior e o menor valor numérico de óbitos novos
obitos_novos = [obitos_acumulados - obitos_acumulados_anterior for _, _, _, _, _, obitos_acumulados_anterior in dados_covid_lista]

maior_obitos_novos = max(obitos_novos)
menor_obitos_novos = min(obitos_novos)

print("Maior valor de óbitos novos:", maior_obitos_novos)
print("Menor valor de óbitos novos:", menor_obitos_novos)

# k. Criar um dicionário para encontrar municípios por estado
municipios_por_estado = {}
for data, estado, _, municipio, _, _ in dados_covid_lista:
    if estado not in municipios_por_estado:
        municipios_por_estado[estado] = [municipio]
    else:
        municipios_por_estado[estado].append(municipio)

# l. Extrair os dados de Teresina/PI
for data, estado, _, municipio, casos_acumulados, obitos_acumulados in dados_covid_lista:
    if municipio == "Teresina" and estado == "PI":
        print(f"Dados de Teresina/PI em {data}:")
        print(f"Casos acumulados: {casos_acumulados}")
        print(f"Óbitos acumulados: {obitos_acumulados}")
